...
class CNode():
    ...
    def __init__(self, layout, locations, directions, goals, forbidTab, horizon, heu):
        self.layout = layout
        self.locations = locations
        self.directions = directions
        self.goals = goals
        self.forbidTab = forbidTab
    ...
    def STsearch(self, layout, agent_i, init, goal, forbidTab, horizon):
        """
        ...
        def is_forbidden(curr_state, succ_state, curr_t):
            Tmax = len(forbidTab)

            # time exceeded or forbidden
            if curr_t + 1 >= Tmax:
                return False
            if (agent_i, succ_state[0]) in forbidTab[curr_t + 1]:
                return True
        ...
        def get_successors(node):
            ...
            for a in range(4):  # 0: stop, 1: fwd, 2: tl, 3: tr
                ...
                if cost > 9999:  # invalid succ_state
                    continue
                if is_forbidden(curr_state, succ_state, t):
                    continue
    ...
    def get_children(self):
        ...
        for i, c_i in enumerate(collisions):
            if c_i:
                # print(f"collision in step {t} among {[i] + c_i}")
                for j in [i] + c_i:
                    if j not in covered:
                        covered.append(j)
                    else:
                        continue
                    if (j, locs[j]) in self.forbidTab[t]:
                        continue
                    new_forbidTab = deepcopy(self.forbidTab)
                    new_forbidTab[t] = sorted(new_forbidTab[t] + [(j, locs[j])])
                    # print(f"{j} cannot go to {locs[j]} at {t}")
                    new_cnode = CNode(
                        self.layout, self.locations, self.directions, self.goals, new_forbidTab,
    ...
    def __repr__(self):
        return str(self.forbidTab)


def CBS(layout, locations, directions, goals, horizon, heu):
    # H * [ct],
    # one ct in row h: (i, loc) meaning agent i cannot be at loc at T_h
    forbidTab = [[] for _ in range(horizon + 1)]

    visited = []
    q = PriorityQueue()
    q.put(
        CNode(layout, locations, directions, goals, forbidTab,
              horizon=horizon, heu=heu)
    )
    # print(f"\n=== New CBS ===")
    num_tree_nodes = 0
    while not q.empty():
        if len(q.queue) > 50:
            print(False, len(q.queue), num_tree_nodes)
            return False
        ...
        if curr_node.is_feasible():  # no collision within the horizon
            print(True, len(q.queue), num_tree_nodes)
